#!/usr/bin/Rscript
# ABOUT: Generate synthetic voice encoding for daily orange articles
...
orange_queue = readRDS(file = "daily-orange-processing-queue.rds")

if(all(orange_queue$processed)){
...
if(nrow(article_target) > 0L){
  ...
  # get article
  article = paste0("https://dailyorange.com", 
...
raw.p = article %>% html_nodes("p") %>% html_text() %>% trimws()
  # truncate to only article section, drop everything below
  raw.p.truncate = article %>% html_node("[class='article-date']") %>% html_text()
  raw.p = raw.p[-c(which(raw.p == raw.p.truncate):length(raw.p))]

raw.h3 = article %>% html_nodes("h3") %>% html_children() %>% html_text() %>% trimws()
raw.ul = article %>% html_nodes("ul") %>% html_text() %>% trimws()
raw.ol = article %>% html_nodes("ol") %>% html_text() %>% trimws() # pending truncation, bookmark


# Prep: Other ----
update_orange_queue = function(queue_tib = orange_queue, article_tib = article_target){
  queue_tib = queue_tib %>% 
...
if(resp.draft$status_code %in% c(200L, 201L)){
  ...
  saveRDS(object = update_orange_queue(), # update processing queue record
